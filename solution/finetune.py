...
# data tool
import datatool
from datatool import np

# model specific info
from incepv4.inception_v4 import inception_v4
from incepv4.inception_utils import inception_arg_scope as scope
size = 299
ckpt_pth = join('incepv4', 'weights', 'inception_v4.ckpt') # ckpt for official finetuned model
save_path =  join('incepv4', 'finetuned', 'weights') # path to save models during training

val_loss_saved = 1000 # used to export meta graph
...
def _save_meta(path=None):
    graph = tf.Graph()
    with graph.as_default():
        if path is None:
            finetuned_path = join(save_path, 'inception_v4_299_tuned_ncs_BEST{}.ckpt'.format(int(val_loss_saved*10000)))
        else:
            finetuned_path = path
...
if __name__ == "__main__":
    _finetune()
    # sorry, it did not load the path correctly, so you may need to do this:
    # best_path = join(join('incepv4', 'finetuned', 'weights'),
    #           'inception_v4_299_tuned_ncs_BEST{}.ckpt'.format(5418))
